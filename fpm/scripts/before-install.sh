...
[ $? != 0 ] && echo -e "NOT FOUND\nPlease install Java 17 and try again" && exit 2

VERSION=`expr "$JAVA_VERSION" : '.*"\([1-9][0-9][\.-].*\)["_]' | cut -d "." -f1 | cut -d "-" -f1`
echo "$VERSION"
test "$VERSION" "!=" "17" && echo "ERROR: Java 17 is required by Gerrit" && exit 3
